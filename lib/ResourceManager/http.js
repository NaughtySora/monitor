...
class Http extends EventEmitter {
  ...
  track(server) {
    ...
    server.prependOnListener('error', this.#error); // if not other events needs to throw error to replicate default behavior
    server.prependOnceListener('listening', this.#listening);
    server.prependOnceListener('close', this.#close);
  }
